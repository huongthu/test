...
function main(sel)
	if BilNpcSongJinShop == nil then
		Talk(1, "", "Library Bill4i3n.lua cã vÊn ®Ò, yªu cÇu qu¶n lý kiÓm tra gÊp.")	
		return
	end
	...
	if (state == 0 or state == 1) then
		...
		Talk(1,"",BilNpcSongJinShop.."PhÝa tr­íc chiÕn tr­êng ®ang trong giai ®o¹n tranh ®o¹t quyÕt liÖt, c¸c vÞ vui lßng tr¸nh ®i mét xÝu.")
		SubWorld = tempSubWorld;
		return
	end;
	SubWorld = nOldSW;	
...
function maintalk()
	...
	local BilSongJinszTitleMsg = BilNpcSongJinShop.."N¬i ®©y lµ chiÕn tr­êng, ng­¬i cÇn ta gióp c¸i g×?"
	local AddOption = {}
...
function xunzhang_exchange()
	if( GetLevel() < 40 ) then
		Talk( 1, "", "Quan TiÕp LiÖu : chØ ca 50 cÊp trë lªn ng­êi ch¬i míi ca thÓ nhËn lÊy huy ch­¬ng");
		return 0
	elseif ( GetExtPoint(0)==0 ) then
		Talk( 1, "", "Quan TiÕp LiÖu : ChØ ca ®· sung tr~ gi¸ ng­êi ch¬i míi ca thÓ nhËn lÊy huy ch­¬ng");
		return 0
	elseif ( CalcFreeItemCellCount() < 1 ) then
		Talk( 1, "", "Xin mêi chuÈn b~ mét chç trèng tíi ®Ó 1 c¸i huy ch­¬ng");
		return 0;
	else
		Say("Quan TiÕp LiÖu : Ng­¬i nghÜ dïng 500 tUch ph©n ®æi lÊy huy ch­¬ng sao?", 2,"§æi lÊy huy ch­¬ng/xunzhang_do", "Kh«ng muèn/no");
	end
end
...
function exp_exchange()
	if( GetLevel() < 40 ) then
		...
		if (GetTiredDegree() == 2) then
			Say(BilNpcSongJinShop.."H«m nay ta mÖt råi, khi kh¸c h·y ®Õn nhÐ!",0);
		else
			local tbOpt = 
			{
			...
			Say(BilNpcSongJinShop.."Ng­¬i muèn ®æi bao nhiªu ®iÓm tÝch lòy thµnh kinh nghiÖm", getn(tbOpt), tbOpt);
		end;
	end
end;
...
function wantpay(mark)
	...
	if( mark > nt_getTask(747) ) then
		Say(BilNpcSongJinShop.."Ngµi ch­a ®ñ ®iÓm, kh«ng thÓ ®æi.", 0);
	elseif (mark == 0) then
		Say(BilNpcSongJinShop.."§· kh«ng cã ®iÓm tÝch lòy mµ còng ®æi kinh nghiÖm, thËt lµ hoang ®­êng.", 0);
	else
		-- local level = GetLevel();
		-- local bonus = bt_exchangeexp(level, mark)
		local bonus = Bil_SongJinExChangeExp(mark)
		Say(BilNpcSongJinShop.."Ngµi cã thÓ ®æi <color=yellow>"..mark.."<color> ®iÓm tÝch lòy lÊy <color=green>"..floor(bonus).."<color> §iÓm kinh nghiÖm kh«ng thÓ céng dån, x¸c ®Þnh ®æi ph¶i kh«ng?", 2, "§ång ý ®æi/#paymark("..mark..")", "Uh, ®Ó cho ta suy nghÜ l¹i mét chót/no")
	end	
end
...
function yuewang_want()
	Say(BilNpcSongJinShop.."TËp trung tinh hoa cña Nh¹c v­¬ng hån th¹ch cã thÓ chÕ t¹o ra Nh¹c v­¬ng kiÕm, cÇn "..YUEWANGHUN_STONECOUNT.." nh¹c v­¬ng hån th¹ch nhÊt ®Þnh ®æi ph¶i kh«ng?", 2, "Muèn/yuewang_change", "Kh«ng muèn/no")
end

function yuewang_change()
	Say(BilNpcSongJinShop.."Nh¹c v­¬ng kiÕm chiÕm kh«ng gian <color=yellow>6 (2 X 3)<color> chç trèng, ng­¬i x¸c ®Þnh hµnh trang cßn chç trèng chø?", 2, "Ch¾c råi!/yuewang_sure", "§Ó ta kiÓm tra l¹i/no")
end
...
function yuewang_sure()
	if (CalcEquiproomItemCount(4, 507, 1, -1) >= YUEWANGHUN_STONECOUNT) then
		...
		AddEventItem(195)
		Say(BilNpcSongJinShop.."C¸i nµy lµ Nh¹c v­¬ng kiÕm, mét tÝn vËt quan träng cña bang héi ®Êy!", 0)
		Msg2Player("§· nhËn ®­îc Nh¹c v­¬ng kiÕm!")
	else
		Say(BilNpcSongJinShop.."Ng­¬i kh«ng ®ñ Nh¹c v­¬ng hån th¹ch, mau kiÓm tra l¹i, Nh¹c v­¬ng kiÕm kh«ng ph¶i ai còng cã thÓ së h÷u.", 0)
	end
end
