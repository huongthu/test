...
function wushuangmengjiang()
	Say("V× khen ngîi ng­êi cã c«ng lao ë chiÕn tr­êng, ng­êi nµo cã c«ng ¾t cã th­ëng.", 6, 
		"Ta muèn xem tr­íc c¸c danh hiÖu vµ vßng s¸ng/Bil_PreViewTitle", 
...
function Bil_PreViewTitle()
	Say("Xin chän danh hiÖu cÇn xem!", 4, "V« song m·nh t­íng §¹i t­íng/#Bil_TitleTop(1)", "V« song m·nh t­íng Phã t­íng/#Bil_TitleTop(2)", "V« song m·nh t­íng Thèng lÜnh/#Bil_TitleTop(3)", "Th«i, thø nµy kh«ng dµnh cho ta/OnCanel")
end

function Bil_TitleTop(Value)
	local BilPreTitle
	if Value == 1 then
		BilPreTitle = "<link=image:\\spr\\skill\\others\\title_dg.spr>Danh HiÖu: <link> <color=green>V« song m·nh t­íng ®¹i t­íng<color>"
	elseif Value == 2 then
		BilPreTitle = "<link=image:\\spr\\skill\\others\\title_ab.spr>Danh HiÖu: <link> <color=green>V« song m·nh t­íng Phã t­íng<color>"
	elseif Value == 3 then
		BilPreTitle = "<link=image:\\spr\\skill\\others\\title_zw.spr>Danh HiÖu: <link> <color=green>V« song m·nh t­íng Thèng lÜnh<color>"
	end
	CreateNewSayEx(BilPreTitle, {{"Xem danh hiÖu kh¸c", Bil_PreViewTitle}, {"KÕt thóc ®èi tho¹i", OnCanel}})
end
