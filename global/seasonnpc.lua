...
local nNum     = GetTask(ID_TASKLINK_LIMITNUM);
	if nNowDate~=nOldDate then
		...
		if nNum>=40 then
			Say("<color=Orange>D· tÈu: <color>Ha! Ha! VŞ thiÕu hŞªp nµy! Mçi ngµy lµm 40 lÇn lµ ®ñ råi! Ngµy mai trë l¹i nhĞ!", 0);
			return 0;
		end;
...
function Task_NewVersionAward()
	...
	local _bilTbAwardNumSeason = {
		[10] = {
			{ValItem = {6, 1, 71, 0, 0, 0}, NameItem = "Tiªn th¶o lé",},
			{ValItem = {6, 1, 23, 0, 0, 0}, NameItem = "ThiÕt la h¸n",},
			{ValItem = {6, 1, 400, 90, random(0,4), 0, 0}, NameItem = "S¸t thñ gi¶n",},
			{ValItem = {6, 1, 910, 0, 0, 0, 0}, NameItem = "Bao D­îc hoµn",},
			{ValItem = {6, 1, 72, 0, 0, 0, 0}, NameItem = "Thiªn s¬n B¶o Lé",},
			-- {ValItem = {6, 1, 125, 0, 0, 0, 0}, NameItem = "QuÕ Hoa Töu",},
			{ValItem = {6, 1, 122, 0, 0, 0, 0}, NameItem = "Phóc Duyªn Lé (TiÓu)",},
		},
		[20] = {
			-- {ValItem = {4, 489, 0, 0, 0, 0, 0}, NameItem = "LÖnh bµi Phong L¨ng §é",},
			-- {ValItem = {6, 1, 147, 1, 0, 0, 0}, NameItem = "HuyÒn tinh kho¸n th¹ch cÊp 1",},
			{ValItem = {6, 1, 23, 0, 0,0, 0}, NameItem = "ThiÕt la h¸n",},
			-- {ValItem = {6, 1, 400, 90, random(0,4), 0, 0}, NameItem = "S¸t thñ gi¶n",},
			{ValItem = {6, 1, 910, 0, 0, 0, 0}, NameItem = "Bao D­îc hoµn",},
			{ValItem = {6, 1, 72, 0, 0, 0, 0}, NameItem = "Thiªn s¬n B¶o Lé",},
			-- {ValItem = {6, 1, 125, 0, 0, 0, 0}, NameItem = "QuÕ Hoa Töu",},
		...
		[30] = {
			{ValItem = {6, 1, 123, 0, 0, 0, 0}, NameItem = "Phóc Duyªn Lé (Trung)",},
			{ValItem = {6, 1, 23, 0, 0,0, 0}, NameItem = "ThiÕt la h¸n",},
			{ValItem = {4, 489, 0, 0, 0, 0, 0}, NameItem = "LÖnh bµi Phong L¨ng §é",},
			-- {ValItem = {6, 1, 1308, 0, 0, 0, 0}, NameItem = "M¶nh Bæ Thiªn Th¹ch (tiÓu)",},
			{ValItem = {6, 1, 400, 90, random(0,4), 0, 0}, NameItem = "S¸t thñ gi¶n",},
			{ValItem = {6, 1, 910, 0, 0, 0, 0}, NameItem = "Bao D­îc hoµn",},
			{ValItem = {6, 1, 72, 0, 0, 0, 0}, NameItem = "Thiªn s¬n B¶o Lé",},
			-- {ValItem = {6, 1, 125, 0, 0, 0, 0}, NameItem = "QuÕ Hoa Töu",},
		...
		[40] = {
			-- {ValItem = {6, 1, 147, 2, 0, 0, 0}, NameItem = "HuyÒn tinh kho¸n th¹ch cÊp 2",},
			{ValItem = {6, 1, 123, 0, 0, 0, 0}, NameItem = "Phóc Duyªn Lé (Trung)",},
			{ValItem = {4, 239, 0, 0, 0, 0, 0}, NameItem = "Tö Thñy Tinh",},
			{ValItem = {4, 238, 0, 0, 0, 0, 0}, NameItem = "Lam Thñy Tinh",},
			{ValItem = {4, 240, 0, 0, 0, 0, 0}, NameItem = "Lôc Thñy Tinh",},
			{ValItem = {6, 1, 400, 90, random(0,4), 0, 0}, NameItem = "S¸t thñ gi¶n",},
			{ValItem = {6, 1, 72, 0, 0, 0, 0}, NameItem = "Thiªn s¬n B¶o Lé",},
			{ValItem = {6, 1, 23, 0, 0,0, 0}, NameItem = "ThiÕt la h¸n",},
			{ValItem = {6, 1, 73, 0, 0, 0, 0}, NameItem = "B¸ch Qu¶ Lé",},
			-- {ValItem = {4, 489, 0, 0, 0, 0, 0}, NameItem = "LÖnh bµi Phong L¨ng §é",},
			{ValItem = {6, 1, 71, 0, 0, 0}, NameItem = "Tiªn th¶o lé",},
			{ValItem = {6, 1, 910, 0, 0, 0, 0}, NameItem = "Bao D­îc hoµn",},
			-- {ValItem = {6, 1, 125, 0, 0, 0, 0}, NameItem = "QuÕ Hoa Töu",},
	...
	-- local a = ((GetLevel() >= 70) and 1 or nil)
	local a = 1
	...
	local bilMsg4Player2Server = "<color=yellow>Ng­êi ch¬i %s hoµn thµnh NV D· tÈu thø %d trong ngµy, nhÉu nhiªn nhËn ®­îc %s"..(a and " vµ %d EXP" or "").."!<color>"
	if ((nNum - nCancelNum) == 10) or ((nNum - nCancelNum) == 20) or ((nNum - nCancelNum) == 30) or ((nNum - nCancelNum) == 40) then
		local bilNumSCom = (nNum - nCancelNum)
		local bilRandomAwardTb = random(1, getn(_bilTbAwardNumSeason[bilNumSCom]))
		AddItem(unpack(_bilTbAwardNumSeason[bilNumSCom][bilRandomAwardTb].ValItem))
		
		if a then
			AddOwnExp(_bilTbAwardNumSeasonEXP[bilNumSCom][1])
		end
		
		Msg2SubWorld(format(bilMsg4Player2Server, GetName(), bilNumSCom, _bilTbAwardNumSeason[bilNumSCom][bilRandomAwardTb].NameItem, _bilTbAwardNumSeasonEXP[bilNumSCom][1]))
		AddLocalNews(format(bilMsg4Player2Server, GetName(), bilNumSCom, _bilTbAwardNumSeason[bilNumSCom][bilRandomAwardTb].NameItem, _bilTbAwardNumSeasonEXP[bilNumSCom][1]))
	end	
end


function main()	
	if bilNPCDaTau ~= 1 then
		return Msg2Player("<color=yellow>H«m nay ta bËn, ngµy mai ng­¬i h·y ®Õn vËy!")
	end
	tasklink_entence();
end;
...
if (tl_gettaskcourse() == 2) then
	...
	if (tl_gettaskcourse() == 0) then
		...
		Say("<color=Orange>D· tÈu: <color>VŞ "..GetPlayerSex().." nµy xem ra ®· b«n ba giang hå ®­îc mét thêi gian dµi råi nhØ, cã muèn tham gia kh¶o nghiÖm nhiÖm vô liªn tôc cña ta kh«ng?",3,"§­îc th«i! Ta kh«ng tin cã nhiÖm vô nµo lµm khã dÔ ®­îc ta/Task_Confirm","Ta muèn biÕt kh¶o nghiÖm cña ng­¬i nãi cã néi dung ra sao/Task_Info","Ta bËn råi, kh«ng r¶nh ngåi t¸n gÉu víi «ng/Task_Exit");
	end
	...
	if (tl_gettaskcourse() == 3) then
		...
		Say("<color=Orange>D· tÈu: <color>VŞ "..GetPlayerSex().."®· hoµn thµnh <color=yellow>"..myCountTimes.."<color>, nhiÖm vô nµy<enter>ngµy h«m nay cã thÓ thùc hiÖn l¹i<color=yellow> "..(40 - GetTask(ID_TASKLINK_LIMITNUM)).."<color>, ng­¬i cã muèn <enter>lµm n÷a kh«ng?",2,"§­¬ng nhiªn, mau cho ta biÕt nhiÖm vô tiÕp theo lµ g× /Task_TaskProcess","§Ó ta nghØ ng¬i mét l¸t ®·! Ta bËn råi/Task_Wait");
...
if myTaskTotalNum==0 or myTaskTotalNum==nil then
	myTitleText = "<color=Orange>D· tÈu: <color>"..myTaskInfo;
else
	myTitleText = "<color=Orange>D· tÈu: <color>§©y lµ nhiÖm vô thø <color=green>"..myTaskTotalNum.."<color>, "..myTaskInfo;
end;
...
Say(myTitleText,
	...
	"Ta muèn hoµn thµnh ngay nhiÖm vô nµy!/bilCompleteNowTask",
...
function bilCompleteNowTask()
	return Talk(1, "bilCompleteNowTask_Give", "§Ó hoµn thµnh ngay nhiÖm vô nµy, ®¹i hiÖp cÇn ph¶i bá ra <color=yellow>1 Tö Thñy Tinh<color> ®Ó lµm tin th× ta míi gióp ®­îc!")
end

function bilCompleteNowTask_Give()
	return GiveItemUI("§Æt vµo 1 Tö Thñy Tinh cho D· TÈu", "NÕu ®¹i hiÖp muèn hoµn thµnh ngay nhiÖm vô th× cø ®Æt 1 Tö Thñy Tinh vµo ®©y cho ta!", "bilCompleteNowTask_GiveOK", "OnCancel");
end

function bilCompleteNowTask_GiveOK(bilCount)
	if bilCount ~= 1 then
		return Say("<color=Orange>D· tÈu: <color>Giµ nµy chØ cÇn 1 Tö Thñy Tinh lµ ®ñ, ng­¬i ®Şnh qua m¾t ta sao?", 2, "Ta nhÇm, ®Ó ta ®Æt l¹i/bilCompleteNowTask_Give", "§Ó ta ®i lÊy vËt phÈm, sÏ quay l¹i sau/OnCancel")
	end
	bilIdxItem = GetGiveItemUnit(bilCount)
	if (GetItemStackCount(bilIdxItem) > 1) then
		return Say("<color=Orange>D· tÈu: <color>Giµ nµy chØ cÇn 1 Tö Thñy Tinh lµ ®ñ, ng­¬i ®Şnh qua m¾t ta sao?", 2, "Ta nhÇm, ®Ó ta ®Æt l¹i/bilCompleteNowTask_Give", "§Ó ta ®i lÊy vËt phÈm, sÏ quay l¹i sau/OnCancel")
	end;
	bilG, bilD, bilP = GetItemProp(bilIdxItem)
	if (bilD ~= 239 or bilG ~= 4) then
		return Say("<color=Orange>D· tÈu: <color>Giµ nµy chØ cÇn 1 Tö Thñy Tinh lµ ®ñ, ng­¬i ®Şnh qua m¾t ta sao?", 2, "Ta nhÇm, ®Ó ta ®Æt l¹i/bilCompleteNowTask_Give", "§Ó ta ®i lÊy vËt phÈm, sÏ quay l¹i sau/OnCancel")
	end
...
if (myTaskCancel==0) then
	...
	Say("<color=Orange>D· tÈu: <color>HiÖn t¹i b¹n kh«ng cã c¬ héi nµo ®Ó hñy bá nhiÖm vô, b¹n chØ cã thÓ lµm l¹i tõ ®Çu th«i! §ång thêi phÇn th­ëng tİch lòy cña b¹n lóc tr­íc sÏ ®­îc tİnh l¹i tõ ®Çu.",
		3,
	...
	Say("<color=Orange>D· tÈu: <color>HiÖn t¹i b¹n cßn "..myTaskCancel.." sè lÇn c¬ héi hñy bá nhiÖm vô, b¹n x¸c ®Şnh hñy bá nhiÖm vô lÇn nµy ®óng kh«ng?",
		2,
...
Say("<color=Orange>D· tÈu: <color>B¹n suy nghÜ kü hñy bá nhiÖm vô lÇn nµy ®óng kh«ng?",2,"§õng l«i th«i n÷a! ta kh«ng muèn lµm nhiÖm vô quû qu¸i nµy ®©u/#Task_Cancel(1)","Th«i ®Ó ta suy nghÜ l¹i ®·!/Task_Wait");
...
function Task_TotalCancel_Main(nCount)
	...
	if nCount~=1 then
		Say("<color=Orange>D· tÈu: <color>Ng­¬i ®­a g× cho ta vËy? Ta chØ cÇn <color=yellow>1 viªn Lôc Thñy Tinh<color> th«i!", 0);
		return
	end;
...
--	Say("Ò°ÛÅ£ºÄãÏÖÔÚ½øĞĞµ½ÁËµÚ "..myTaskLoops.." »·ÖĞµÄµÚ "..myTaskLinks.." Á´ÖĞµÄµÚ "..myTaskTimes.." ´Î¡£<enter>ÄãÁ¬Ğø½øĞĞµÄ´ÎÊıÎª£º"..tl_counttasklinknum(1).." ´Î<enter>ÄãÁ¬Ğø½øĞĞµÄÁ´ÊıÎª£º"..tl_counttasklinknum(2).." Á´<enter>"..myMainValueText1,0);
	...
	Say("<color=Orange>D· tÈu: <color>HiÖn t¹i ng­êi ®· hoµn thµnh <color=yellow>"..myCountTimes.."<color> sè lÇn nhiÖm vô ta giao cho, cÇn cè g¾ng h¬n nhĞ!", 0);
...
function Task_Accept()
	...
	if (CalcFreeItemCellCount() < 5) then
		Say("<color=Orange>D· tÈu: <color>Hµnh trang ®· ®Çy, h·y s¾p xÕp l¹i cho ng¨n n¾p.",0);
		return
	end;
local myTaskType = tl_getplayertasktype()
	...
	elseif (myTaskType == 6) then
		...
		Say("<color=Orange>D· tÈu: <color>Nh÷ng thø ng­¬i giao cho ta ch­a ®¹t ®óng yªu cÇu, cè g¾ng h¬n nhĞ!",0);
	end
...
if ( nCount > 1 ) then
	Say("<color=Orange>D· tÈu: <color>VŞ nµy"..GetPlayerSex()..", Ng­¬i bá nhiÒu ®å v« nh­ vËy xem tíi ta hoa c¶ m¾t, tõ tõ th«i nµo!",0);
	return 0
elseif ( nCount == 0) then
	Say("<color=Orange>D· tÈu: <color>VŞ nµy"..GetPlayerSex()..", ng­¬i cã thËt ®· bá vµo thø ta cÇn kh«ng? Kh«ng ph¶i l·o phu hoa m¾t chø?",0);
	return 0
end
...
--	tl_print ("Ò°ÛÅ¼ìÑéÁËÎïÆ·£º"..ItemGenre..DetailType..ParticularType..nSeries..Level)
	...
	if (tl_checktask(myTaskGoods) == 1) then
		...
		Say("<color=Orange>D· tÈu: <color>Nh÷ng viÖc b¹n lµm ch­a ®¹t ®óng yªu cÇu, cè g¾ng h¬n nhĞ!",0);
	end
end
...
function Task_Accept_06()
	...
	if (tl_checktask()==1) then
		...
		Say("<color=Orange>D· tÈu: <color>Hahaha! VŞ "..GetPlayerSex().." nµy, ta tuy bÊt tµi, nh÷ng còng hiÓu ®­îc ch÷ tİn trªn giang hå, ng­¬i cßn ch­a thu thËp ®ñ m¶nh s¬n Hµ X· T¾c mµ ta yªu cÇu sao cã thÓ ®Õn l·nh th­ëng ®©y?",0);
		return 0
	end
...
if _CancelTaskDebug()~=1 then
	Say("<color=Orange>D· tÈu: <color>Uhm! B¹n trÎ nµy h×nh nh­ kh«ng cßn c¬ héi hñy bá ", 0);
	return
end;
...
if checkTask_Limit()~=1 then return end;
	...
	if (nType==2) then
		if (myMapNum>=100) then
			...
			Say("<color=Orange>D· tÈu: <color>ng­¬i cã ®óng ®· mang <color=yellow>100<color> m¶nh s¬n Hµ X· T¾c kh«ng? Ta cã nh×n lÇm kh«ng vËy?",0);
			return
		end;
	end;
...
function Task_Exit()
	...
	if (GetSex() == 0) then
		Say("<color=Orange>D· tÈu: <color>Hahaha! §îi sau khi vŞ ®¹i hiÖp nµy th«ng qua kh¶o nghiÖm xong nhËn ®­îc c¸c phÇn th­ëng phong phó, tù nhiªn l¹i biÕt ®Õn t×m ta th«i!",0);
	else
		Say("<color=Orange>D· tÈu: <color>Hahaha! §îi sau khi vŞ n÷ hiÖp nµy th«ng qua kh¶o nghiÖm xong nhËn ®­îc c¸c phÇn th­ëng phong phó, tù nhiªn l¹i biÕt ®Õn t×m ta th«i!",0);
	end
...
function Task_Punish()
	Say("<color=Orange>D· tÈu: <color>VŞ "..GetPlayerSex().." nµy Cã ph¶i gÊp l¾m kh«ng, kh¶o nghiÖm cña ta s¾p xÕp khã ®Õn nh­ vËy ­? LÇn sau ®Õn vËy!",0);
	return 0
end
...
function Task_GiveAward()
	...
	-- AddOwnExp(1000000)
	-- Msg2Player("<color=orange>D· TÈu: <color><color=yellow>Nhµ ng­¬i vÊt v¶ qu¸, ta  th­ëng thªm cho ng­¬i 1 triÖu Kinh nghiÖm!")
	
	if GetTask(5905) == 61 then
...
function Task_AwardRecord()
	...
	myCountTimes = tl_counttasklinknum(1)
	Ladder_NewLadder(10006, GetName(), myCountTimes,1);
end
...
----------------------------------------------------- FIX Huyen Tinh -----------------------------------------------------
	if myGenre == 6 and myDetail == 1 and myParticular == 147 then
		myParticular = 71
	end
----------------------------------------------------- FIX Huyen Tinh -----------------------------------------------------
...
if (tl_gettaskcourse() == 3) then
	Say("<color=Orange>D· tÈu: <color>Ng­¬i ®· l·nh th­ëng råi! §Şnh g¹t giµ nµy µ?",0);
	return
end
	--2007-09-19 Ôö¼ÓÎïÆ·½±ÀøÊ±µÄ±³°ü¿Õ¼äÅĞ¶Ï
	if (CalcFreeItemCellCount() < 5) then
...
if (tl_gettaskcourse() == 3) then
	...
	-- nAward = (nAward > 30000) and 30000 or nAward 
