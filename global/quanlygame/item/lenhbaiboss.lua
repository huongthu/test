...
--		Msg2Player("Bang héi b¶n ®å cÊm chØ sö dông")
--		return 1
--	end
...
--			Msg2Player("´ËµØ?»ÄÜÊ¹ÓÃ´ËµÀ¾?);
	...
	for k = 1, j do
		...
		--AddGlobalCountNews("Boss ®· xuÊt hiÖn.!!!. C¸c cao thñ vâ l©m h·y nhanh ch©n ®Õn so tµi nµo..!")
        Msg2SubWorld("Qu¶n TrÞ <color=yellow>"..GetName().." <color=red>®· gäi  <color=green>Event <color=violet> "..GoldenBossName );
		--Msg2SubWorld("<color=yellow>C¸c game thñ h·y mau mau tranh thñ ra s¨n boss nµo..!");  --- ---------------------------------------------------------------------------------------------------------------------toa do-----------------------------------------------------------------------------------------	
	Msg2SubWorld("Täa ®é boss hiÖn t¹i: <color=green>"..SubName.."<color> <color=blue>"..nX.."/"..nY.."<color>");
	--Msg2SubWorld("<color=yellow>ThËp ®¹i hµo kiÖt <color=violet>%s <color>t¸i xuÊt giang hå t¹i <color=green>%s <color>täa ®é <color=blue>(%d,%d)<color> c¸c ®¹i hiÖp h·y mau mau t×m vµ tiªu diÖt boss ®Ó nhËn quµ tri ©n..");
		local szNews = format("<color=yellow>Noel<color=red> ®· xuÊt hiÖn <color=violet>"..GoldenBossName);
...
function GetDesc(nItemIdx)
	...
	return "<color=blue> gäi ra thø 2 c¸ Boss ®Ých tû lÖ<color=yellow>1/"..doubleodds;
end