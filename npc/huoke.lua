...
AddTimer(nNextTime * 18, "CallHuoKeTime", 0) 
	...
	print(format("ÕâÒÔºóÃë»òÕßÐÄhuoke%s",  FormatTime2String("%Y-%m-%d %H:%M:%S", GetCurServerTime() + nNextTime)))
end 
...
function CallHuoKeBoss(nMapId, nPosX, nPosY) 
	...
	local tbNpc = {szName = "HuoKe", nIsboss = 1, nNpcId = 1584, nLevel = 119, nTime = 10, szScriptPath = "\\script\\global\\npc\\huoke.lua"}
...
AddGlobalNews(format("Huoke xuÊt hiÖn ë M¹c B¾c th¶o nguyªn (%d, %d)", nPosX/8, nPosY/16)) 
