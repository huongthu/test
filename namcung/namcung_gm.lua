...
function dmcreateit()
	...
	elseif (oper == 0) and (GetCamp() == 4) and (GetLevel() >= 50) and (GetReputeLevel(GetRepute()) >= 6) and (GetLeadLevel() >= 30) and (HaveItem(195) == 1) then
		...
		i = random(0,1)
		if (i == 0) then
			Talk(1,"", "Kiªm hiÖp ch­ëng m«n nh©n: Nªu nh­ muèn lËp bang héi, ng­¬i v× na bá ra 1 l­îng lín thêi gian, søc lùc cïng t©m huyOt, kh«ng thÓ nöa ®­êng hñy bá." )
		else
...
function dialog_main()
	local szTitle = "<npc>Kiªm hiªp quÇn hïng chiªn nam h¶i,long tranh hæ ®Êu diÖu Cöu ch©u. Ta vèn lµ sø gi¶ ®¹i biÓu t©y s¬n, hoan nghªnh c¸c hßa kiÖt ®a tham gia VLTK Offline nµy ...! Script ®­îc viÖt hãa bëi <color=green>Th«ng Hên <color>, rÊt mong ®c sù ®ãng gãp cña c¸c b¹n..!"
	local tbOpt =
	{
	...
	for i=1, getn(%tbGMAccount) do
		if szAccount == %tbGMAccount[i] then
			tinsert(tbOpt, 1, {"×ªGMÕËºÅ", gm_function})
			...
			tinsert(tbOpt, 1, {"ÎÒÒªÏ´Ëè", clear_attibute_point})
			break
		end
	end
...
function gm_function()
	...
	if CalcFreeItemCellCount() >= 1 then
		local tbItem = {szName="´«ËÝÈ¨ÕÈ", tbProp={6,1,2766,1,0,0}, nBindState=-2}
		tbAwardTemplet:GiveAwardByList(tbItem, "gmÕËºÅ¸ìÈ¡µµ¾ß", 1)
	end
end
